...
function ImagePageContent({ isAdmin }: { isAdmin: boolean }) {
  ...
  const handleTimeoutRetryContinue = useCallback(async (targetTaskId?: string) => {
    const taskId = targetTaskId || timeoutRetry?.taskId;
    const conversationId = timeoutRetry?.conversationId ?? conversationsRef.current.find((conversation) =>
